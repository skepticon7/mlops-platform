import {ClusteringState} from "@/types/model.types";
import {Loader2, Terminal} from "lucide-react";

interface KmeansResultProps {
    prediction: ClusteringState
}


export default function KmeansResult({prediction} : KmeansResultProps) {
    return (
            ...
            <div className="p-5">
                ...
                {prediction.data && !prediction.loading && (
                    <div className="space-y-5">
                        <div>
                            <p className="text-[10px] uppercase tracking-[.07em] text-text-tertiary mb-1.5">Assigned Cluster</p>
                            <p className="text-[32px] font-bold font-mono text-success">{prediction.data.cluster}</p>
                        </div>
                        <div>
                                ...
                                <div key={cls} className="mb-3">
                                    ...
                                    <div className="h-1.5 rounded-full bg-border overflow-hidden">
                                        <div
                                            ...
                                            style={{
                                                width: `${prob * 100}%`,
                                                background: cls === `Cluster ${prediction.data?.cluster}` ? "var(--color-success, #22c55e)" : "var(--color-border-strong, #555)"
                                            }}
                                        />
                                    </div>
                                </div>
